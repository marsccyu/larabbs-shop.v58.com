                            ...
                            @if($product->type === \App\Models\Product::TYPE_CROWDFUNDING)
                                <div class="crowdfunding-info">
                                    ...
                                    <div class="progress-info">
                                        <span class="current-progress">目前進度：{{ $product->crowdfunding->percent }}%</span>
                                        <span class="pull-right user-count">{{ $product->crowdfunding->user_count }}個支持者</span>
                                    </div>
                                    <!-- 如果众筹状态是众筹中，则输出提示语 -->
                                    @if ($product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_FUNDING)
                                        <div>此商品必須在
                                            ...
                                            眾籌活動將在<span class="text-red">{{ $product->crowdfunding->end_at->diffForHumans(now()) }}</span>結束！
                                        </div>
                                    @endif
                                </div>
                        @else
                            <!-- 原普通商品模块开始 -->
                                ...
                                <div class="sales_and_reviews">
                                    ...
                                    <div class="review_count">累計評論<span class="count">{{ $product->review_count }}</span></div>
                                    <div class="rating" title="評分 {{ $product->rating }}">评分
                        ...
                        <!-- 眾籌商品區塊結束 -->
                            ...
                            <!-- 众筹商品下单按钮开始 -->
                                @if($product->type === \App\Models\Product::TYPE_CROWDFUNDING)
                                    @if(Auth::check())
                                        @if($product->crowdfunding->status === \App\Models\CrowdfundingProduct::STATUS_FUNDING)
                                            <button class="btn btn-primary btn-crowdfunding">參與眾籌</button>
                                        @else
                    ...
                    <div class="product-detail">
                        <ul class="nav nav-tabs" role="tablist">
                            <li role="presentation" class="active">
                                <a href="#product-detail-tab" aria-controls="product-detail-tab" role="tab" data-toggle="tab">商品詳情</a>
                            </li>
                            <li role="presentation">
                                <a href="#product-reviews-tab" aria-controls="product-reviews-tab" role="tab" data-toggle="tab">用戶評價</a>
                            </li>
                        </ul>
                        <div class="tab-content">
                            <div role="tabpanel" class="tab-pane active" id="product-detail-tab">
                                ...
                                <div class="properties-list">
                                    <div class="properties-list-title">產品參數：</div>
    ...
    <script>
        $(document).ready(function () {
            ...
            $('.btn-favor').click(function () {
                ...
                axios.post('{{ route('products.favor', [
                        'product' => $product->id
                    ]) }}')
